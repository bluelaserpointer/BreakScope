...
public class Belt extends GimmickListener{
	...
	int[] beltSpeed; //ベルトスピード0~50
	double[] direction; //ベルト方向
	...
	public void loadStarted(){ //変数初期化
		beltSpeed = new int[bs.stageGridTotal];
		direction = new double[bs.stageGridTotal];
	}
	@Override
	public void gamePaint(int grid,boolean inScreen){ //基本処理
		if(!inScreen) //スクリーン外のとき処理放棄
			return;
		this.editorPaint(grid,true);
	}
	public void editorPaint(int grid,boolean inScreen){
		...
		final double angle = direction[grid]; //回転角度
		bs.g2.rotate(angle,x,y);
		final int x2 = x + (bs.nowFrame*beltSpeed[grid])%25;
		for(int k = -3;k < 2;k++)
			bs.drawImageBS(bs.gimmickImg[bs.gimmickKind[grid]],x2 + k*25,y - 50);
		bs.g2.rotate(-angle,x,y);
		bs.g2.setClip(CLIP);
