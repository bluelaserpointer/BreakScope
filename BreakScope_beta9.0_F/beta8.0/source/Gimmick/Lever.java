...
public class Lever extends GimmickListener{
	...
	static HashMap<String,ArrayList<Integer>> gimmickFrequencyList = new HashMap<String,ArrayList<Integer>>(); //周波数と使用ギミック座標ID
	static BitSet gimmickSwitch; //入力状態
	...
	public void loadStarted(){ //変数初期化
		...
		gimmickSwitch = new BitSet(bs.stageGridTotal);
	}
	@Override
	public void gamePaint(int grid,boolean inScreen){ //基本処理
		final int x = grid/bs.stageGridH*100 + 50,y = grid%bs.stageGridH*100 + 50; //座標情報変換
		if(abs(bs.playerX - x) < 50 && abs(bs.playerY - y) < 50){ //自機が接触範囲内
			bs.drawImageBS(key_space,x - 50,y - 50); //スペースキーで操作することを伝える
			if(bs.actioned && !frequency[grid].equals("")){ //操作された
				final ArrayList<Integer> groups = gimmickFrequencyList.get(frequency[grid]); //同じ周波数のギミックのIDを取得
				if(groups != null){
		...
		bs.drawImageBS(bs.gimmickImg[bs.gimmickKind[grid]],x - 50,y - 50,100,100);
	}
	public void editorPaint(int grid,boolean inScreen){ //エディター用描画処理
		if(!inScreen) //スクリーン外のとき処理放棄
			return;
	...
	public static void addFrequency(int id,String _frequency){ //周波数追加処理
