...
public class TANK extends EnemyListener{
	...
	public void construct(BreakScope bs){
		...
		tankenemyDeathSE = new SoundClip("source/media/sen_ta_dame_gion02.wav");
	...
	public void gamePaint(int id,double x,double y,double angle,boolean aimed,int readyTime,int playerDistance){ //基本処理
		...
		if(aimed){
			...
			if(aimed){
				if(readyTime >= 100){
					bs.setDefaultEnemyBullet(id,"ARTILLERY",x,y,5,35,1);
		...
		bs.g2.drawImage(img,x_int - img.getWidth(null)/2,y_int - img.getHeight(null)/2,bs);
		bs.g2.rotate(-angle2,x_int,y_int);
		bs.g2.rotate(angle,x_int,y_int); //砲塔描画
		final Image img2 = bs.enemyImg[bs.enemyKind[id]];
		bs.g2.drawImage(img2,x_int - img2.getWidth(null)/2,y_int - img2.getHeight(null)/2,bs);
	...
	@Override
	public void editorPaint(int id,int x,int y,double angle){ //エディター描画処理
		final Image img = tankBodyImg;
	...
	public void damaged(int id,int dmg){ //傷害追加処理
		tankenemyDeathSE.stop();
		tankenemyDeathSE.play(); //攻撃されると効果音が出る
	}
	@Override
