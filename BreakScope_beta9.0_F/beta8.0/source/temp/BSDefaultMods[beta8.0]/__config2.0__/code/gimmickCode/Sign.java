...
public class Sign extends GimmickListener{
	...
	@Override
	public void gamePaint(int map){
		super.defaultPaint(map);
		final int x = map/50*100 + 50,y = map%50*100 + 50;
		if(abs(bs.playerX - x) < 50 && abs(bs.playerY - y) < 50){
			bs.myComment = text[map];
			bs.myCommentFrame = bs.nowFrame;
		}
		/*if(abs(bs.playerX - x) < 50 && abs(bs.playerY - y) < 50 && !read.get(map)){ //範囲内&未読
			if(bs.playerActionTarget == bs.NONE){
				bs.playerActionTarget = bs.GIMMICK + map;
				bs.playerActionProgress = 0;
			}else
				bs.playerActionProgress++;
			if(bs.playerActionProgress >= 100) //読み終わった
				read.set(map);
		}else if(bs.playerActionTarget / 10000 == bs.GIMMICK && bs.playerActionTarget % 10000 == map)
