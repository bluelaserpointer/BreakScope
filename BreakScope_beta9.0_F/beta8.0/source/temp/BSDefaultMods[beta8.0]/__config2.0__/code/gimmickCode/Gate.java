...
public class Gate extends GimmickListener{
	...
	final int[] gateMoving = new int[2500], //ゲート開閉進行度0~50
		direction = new int[2500]; //ゲート開閉方向
	...
	public void gamePaint(int map){ //基本処理
		...
		int move = gateMoving[map]; //進行状況
		if(Lever.gimmickSwitch.get(map)){ //この座標の入力を検出-on
			bs.gimmickHP[map] = MIN;
			if(move <= OPEN)
				move += 2;
		}else{
			bs.gimmickHP[map] = MAX;
			if(move >= CLOSE)
				move -= 2;
		}
		gateMoving[map] = move;
		...
		final Image gateImg = bs.gimmickImg[bs.gimmickKind[map]];
		if(direction[map] == VERTICAL){ //縦
			bs.g2.rotate(PI/2,x,y);
			bs.g2.drawImage(gateImg,x - 100 - move,y - 50,100,100,bs);
			bs.g2.drawImage(gateImg,x + move,y - 50,100,100,bs);
			bs.g2.rotate(-PI/2,x,y);
		}else{
			bs.g2.drawImage(gateImg,x - 100 - move,y - 50,100,100,bs);
			bs.g2.drawImage(gateImg,x + move,y - 50,100,100,bs);
	...
	@Override
	public void editorPaint(int map){
		final int x = map/50*100 + 50,y = map%50*100 + 50; //座標情報に変換
		final Shape clip = bs.g2.getClip(); //もともとのトリミング範囲を保存
		bs.g2.setClip(x - 50,y - 50,100,100);
		final Image gateImg = bs.gimmickImg[bs.gimmickKind[map]];
		if(Lever.gimmickSwitch.get(map)){ //開いている	
			if(direction[map] == VERTICAL){ //縦
				bs.g2.rotate(PI/2,x,y);
				bs.g2.drawImage(gateImg,x - 130,y - 50,100,100,bs);
				bs.g2.drawImage(gateImg,x + 30,y - 50,100,100,bs);
				bs.g2.rotate(-PI/2,x,y);
			}else{
				bs.g2.drawImage(gateImg,x - 130,y - 50,100,100,bs);
				bs.g2.drawImage(gateImg,x + 30,y - 50,100,100,bs);
			...
			if(direction[map] == VERTICAL){ //縦
				bs.g2.rotate(PI/2,x,y);
				bs.g2.drawImage(gateImg,x - 100,y - 50,100,100,bs);
				bs.g2.drawImage(gateImg,x,y - 50,100,100,bs);
				bs.g2.rotate(-PI/2,x,y);
			}else{
				bs.g2.drawImage(gateImg,x - 100,y - 50,100,100,bs);
				bs.g2.drawImage(gateImg,x,y - 50,100,100,bs);
